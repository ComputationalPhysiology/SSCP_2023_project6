...
using the coupled ORd-Land model. The action potential and force development
is triggered by a stimulus current of magnitude lasting from 0 to 0.5 ms. These
are the default parameter settings in the coupled model. 
...
for lmbda in np.linspace(0.9,1.1,5):
    ...
    p = em_model.init_parameter_values(lmbda=lmbda) #, Ca_amplitude=0.6, Ca_diastolic=0.17, tau1=75, tau2=175)
    #initialize and solve the model
    init = em_model.init_state_values(CaTrpn=0.01) #Default value is CaTrpn=0, but gives division by zero error
